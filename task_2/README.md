...
2.Feature Selection & RemovalCabin
----------------------------------------
...
------------------------------------
1.Family Size (FamilySize): Combined isolated relative counts:$$\text{FamilySize} = \text{SibSp} + \text{Parch} + 1
2.Solo Traveler Flag (IsAlone): Created a binary indicator:$$\text{IsAlone} = \begin{cases} 1 & \text{if } \text{FamilySize} = 1 \\ 0 & \text{if } \text{FamilySize} > 1 \end{cases}
3.Explicit String Labels: Translated raw codes (0/1 $\rightarrow$ 'Did Not Survive'/'Survived'; 1, 2, 3 $\rightarrow$ '1st Class', '2nd Class', '3rd Class') for explicit chart legends and axes.
